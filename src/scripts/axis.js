...
export default class Axis {
    constructor(container) {
        ...
        */
        ...
        var canvasSize = constants.tickLength + constants.timescaleThickness;
        var text_size;
        var fontSize;
        var strokeStyle;
        var ctx;
        ...
        */
        function renderBaseLine() {
            if (isHorizontal) {
                if (_position == "bottom") {
                    ctx.fillRect(0, 0, _size, constants.timescaleThickness);
                } else {
                    ctx.fillRect(0, constants.tickLength, _size, constants.timescaleThickness);
                }
            } else {
                if (_position == "right") {
                    ctx.fillRect(0, 0, constants.timescaleThickness, _size);
                } else {
                    ctx.fillRect(constants.tickLength, 0, constants.timescaleThickness, _size);
                }
            }
        }
        ...
        function renderMajorTicks() {
            ...
            for (var i = 0, len = _ticks.length; i < len; i++) {
                ...
                if (isHorizontal) {
                    ...
                    ctx.moveTo(x, 1);
                    ctx.lineTo(x, 1 + constants.tickLength);
                    ...
                    ctx.moveTo(1, x);
                    ctx.lineTo(1 + constants.tickLength, x);
                    ...
                    switch (_position) {
                        case "bottom":
                            for (i = 0; i < len; i++) {
                                ...
                                ctx.moveTo(x, 1);
                                ctx.lineTo(x, 1 + constants.smallTickLength);
                            }
                            break;
                        case "top":
                            for (i = 0; i < len; i++) {
                                x = that.getCoordinateFromTick(smallTicks[i]);
                                ctx.moveTo(x, constants.tickLength - constants.smallTickLength);
                                ctx.lineTo(x, 1 + constants.tickLength);
                            }
                            break;
                        case "left":
                            for (i = 0; i < len; i++) {
                                x = that.getCoordinateFromTick(smallTicks[i]);
                                ctx.moveTo(constants.tickLength - constants.smallTickLength, _size - x - 1);
                                ctx.lineTo(constants.tickLength, _size - x - 1);
                            }
                            break;
                        case "right":
                            for (i = 0; i < len; i++) {
                                ...
                                ctx.lineTo(1 + constants.smallTickLength, _size - x - 1);
                            }
                            break;
                    }
                }
        ...
        */
        function render() {
            ...
            ctx.lineWidth = constants.timescaleThickness;
            if (isHorizontal) {
    ...
    constructor() {
        ...
        this.createTicks = function (range) {
            ...
            if (dx == 2)
                count++;
            for (var i = 0; i < count + 1; i++) {
                var tick_position = dates.getCoordinateFromYMD(x0 + i * dx, 0, 1);
