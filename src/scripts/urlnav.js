export default class UrlNav {
	...
	*/
	static vcelementToNavString(vcElem, vp) {
		var nav = '';
		var el = vcElem;
		while (vcElem) {
			if (vcElem.type) {
	...
	#/t55/e118
	*/
	static navStringTovcElement(nav, root) {
		if (!nav)
			return null;
		try {
			var k = nav.indexOf('@');
	...
	#/t55/e118
	*/
	static navStringToVisible(nav, vc) {
		var k = nav.indexOf('@');
		var w = 1.05;
		var h = 1.05;
		var x = 0;
