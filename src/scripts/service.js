...
export default class Service {
  constructor() {
    ...
    var map = new Map();
    //var constants.serviceUrl = constants.serverUrlHost + '/api/';
    ...
    Object.defineProperties(this, {
      superCollectionName: {
        configurable: false,
        get: function () {
          return superCollectionName;
        },
        set: function (value) {
          superCollectionName = value;
        }
      },
      collectionName: {
        configurable: false,
    ...
    function exportTimelines(topmostTimelineId) {
      ...
      var request = new Request(constants.serviceUrl);
    ...
    function importTimelines(intoTimelineId, newTimelineTree) {
      if (
        typeof intoTimelineId === 'undefined' ||
        typeof newTimelineTree === 'undefined'
      ) {
    ...
    function getRootTimelineId(sc, c) {
      if (typeof sc === 'undefined') sc = superCollectionName;
      if (typeof c === 'undefined') c = collectionName;
      //CZ.Authoring.resetSessionTimer();
      var request = new Request(constants.serviceUrl);
    ...
    function isUniqueCollectionName(proposedCollectionName) {
      ...
      //CZ.Authoring.resetSessionTimer();
      var request = new Request(constants.serviceUrl);
      if (typeof superCollectionName !== 'undefined')
    ...
    function deleteCollection() {
      if (typeof collectionName === 'undefined') return false;
    ...
    function getTour(tourGUID) {
      ...
      //CZ.Authoring.resetSessionTimer();
      var request = new Request(constants.serviceUrl);
      request.addToPath(superCollectionName); // || 'chronozoom');
      if (typeof collectionName !== 'undefined')
        request.addToPath(collectionName);
      request.addToPath('tour');
    ...
    function getSearch(query, scope) {
      ...
      var data = {
        supercollection: superCollectionName,
    ...
    function getBingImages(query, top, skip) {
      if (typeof top === 'undefined') {
        top = constants.defaultBingSearchTop;
      }
      if (typeof skip === 'undefined') {
        skip = constants.defaultBingSearchSkip;
      }
      var request = new Request(constants.serviceUrl);
    ...
    function getEditableCollections(includeMine) {
      ...
      request.addParameter(
        'currentSuperCollection',
        superCollectionName
      );
      request.addParameter('currentCollection', collectionName);
      return $.ajax({
        type: 'GET',
        cache: false,
        dataType: 'json',
  ...
  static getCanEdit() {
    //CZ.Authoring.resetSessionTimer();
    var request = new Request(constants.serviceUrl);
    request.addToPath(this.superCollectionName);
    if (typeof collectionName !== 'undefined')
      request.addToPath(collectionName);
  ...
  static getProfile(displayName) {
    if (typeof displayName === 'undefined') {
      displayName = this._testLogin ? 'anonymous' : '';
    }
    //CZ.Authoring.resetSessionTimer();
    var request = new Request(constants.serviceUrl);
    request.addToPath('user');
  ...
  static getTimelines(r, sc, c) {
    if (typeof sc === 'undefined') sc = this.superCollectionName;
    if (typeof c === 'undefined') c = this.collectionName;
    //CZ.Authoring.resetSessionTimer();
    var request = new Request(constants.serviceUrl);
  ...
  static getCollections(superCollectionName) {
    //CZ.Authoring.resetSessionTimer();
    var request = new Request(constants.serviceUrl);
    request.addToPath(superCollectionName);
  ...
  static getCollection() {
    //CZ.Authoring.resetSessionTimer();
    var request = new Request(constants.serviceUrl);
    console.log(this.superCollectionName);
    request.addToPath(this.superCollectionName);
    if (typeof collectionName !== 'undefined')
      request.addToPath(collectionName);
    request.addToPath('data');
  ...
  constructor() {
    ...
    function timeline(t) {
      return {
        ...
        start: dates.getDecimalYearFromCoordinate(t.x),
        FromIsCirca: t.FromIsCirca,
        end: typeof t.endDate !== 'undefined' ?
          t.endDate : dates.getDecimalYearFromCoordinate(t.x + t.width),
    ...
    function contentItem(ci) {
      return {
        id: ci.guid,
        ParentExhibitId: ci.contentItem ?
          ci.contentItem.ParentExhibitId : ci.ParentExhibitId,
        title: ci.contentItem ? ci.contentItem.title : ci.title,
        description: ci.contentItem ?
          ci.contentItem.description : ci.description,
        uri: ci.contentItem ? ci.contentItem.uri : ci.uri,
        mediaType: ci.contentItem ? ci.contentItem.mediaType : ci.mediaType,
        attribution: ci.contentItem ?
          ci.contentItem.attribution : ci.attribution,
        mediaSource: ci.contentItem ?
          ci.contentItem.mediaSource : ci.mediaSource,
